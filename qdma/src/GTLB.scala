...
class GTLB extends Module with BaseTLB{
	...
	val base_page 			= RegInit(UInt(64.W),0.U)
	...
	val h2c_page 			= io.h2c_in.bits.addr - base_page
	val h2c_index			= h2c_page >> PAGE_SHIFT
	val h2c_outrange		= (h2c_page.asSInt < 0.S) | (h2c_page >= (wrtlb_index << PAGE_SHIFT))
	val c2h_page 			= io.c2h_in.bits.addr - base_page
	val c2h_index			= c2h_page >> PAGE_SHIFT
	val c2h_outrange		= (c2h_page.asSInt < 0.S) | (c2h_page >= (wrtlb_index << PAGE_SHIFT))
	...
	when(io.wr_tlb.fire){
		...
		when(io.wr_tlb.bits.is_base===1.U){
			base_page			:= Cat(io.wr_tlb.bits.vaddr_high, io.wr_tlb.bits.vaddr_low)
