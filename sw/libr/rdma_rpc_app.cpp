...
constexpr size_t rpc_resp_size = 2048;
...
void sub_task_server(int thread_index, int record_index, QpHandler *handler, void *buf, size_t ops) {
	...
	OffsetHandler recv_comp(NUM_PACK, PACK_SIZE, 0);
	...
	constexpr size_t delay_cycles = 2200 * 130; // 1us = 2200 cycles, delay 130 us
	std::queue<size_t> delay_queue;
	...
	while (!done && !stop_flag) {
		...
		size_t now_rdtsc = __rdtsc();
		while (!delay_queue.empty() && now_rdtsc - delay_queue.front() > delay_cycles) {
			delay_queue.pop();
			post_send(*handler, send.offset(), rpc_resp_size);
			send.step();
		}
...
void sub_task_client(int thread_index, int record_index, QpHandler *handler, void *buf, size_t ops) {
	...
	while (recv_comp.index() < ops && !stop_flag) {
		ne_send = poll_send_cq(*handler, wc_send);
		if (ne_send != 0) {
			global_timer.start_once();
		}
		...
		for (int i = 0;i < ne_recv;i++) {
			assert(wc_recv[i].status == IBV_WC_SUCCESS);
			assert(wc_recv[i].byte_len == static_cast<uint32_t>(rpc_resp_size));
...
int main(int argc, char *argv[]) {
	...
	if (RECORD_FLAG && FLAGS_nodeId == 0) {
		...
		std::ofstream outfile("/home/cxz/rdma_rpc_app.csv");
		for (size_t i = 0;i < pkts_info.size();i++) {
		...
		outfile.close();
		std::cout << "Recorded data saved to /home/cxz/rdma_rpc_app.csv" << std::endl;
	}
