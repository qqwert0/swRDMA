...
class QDMABlackBox(VIVADO_VERSION:String, PCIE_WIDTH:Int, SLAVE_BRIDGE:Boolean, AXI_BRIDGE:Boolean) extends BlackBox{
	val io = IO(new Bundle{
		val sys_rst_n 					= Input(Bool())
		val sys_clk 					= Input(Clock())
		val sys_clk_gt 					= Input(Clock())

		val pci_exp_txn					= Output(UInt(PCIE_WIDTH.W))
		val pci_exp_txp					= Output(UInt(PCIE_WIDTH.W))
		val pci_exp_rxn					= Input(UInt(PCIE_WIDTH.W))
		val pci_exp_rxp					= Input(UInt(PCIE_WIDTH.W))
