...
class CusHeadProcess() extends Module{
	...
	switch(io.swrdma_head_choice){
		...
		is(1.U){
            io.meta_out.bits.pkg_length  := meta_fifo.io.out.bits.pkg_length - (CONFIG.SWRDMA_HEADER_LEN1/8).U
		...
		is(2.U){
            io.meta_out.bits.pkg_length  := meta_fifo.io.out.bits.pkg_length - (CONFIG.SWRDMA_HEADER_LEN2/8).U
		...
		is(3.U){
            io.meta_out.bits.pkg_length  := meta_fifo.io.out.bits.pkg_length - (CONFIG.SWRDMA_HEADER_LEN3/8).U
		...
		is(4.U){
            io.meta_out.bits.pkg_length  := meta_fifo.io.out.bits.pkg_length - (CONFIG.SWRDMA_HEADER_LEN4/8).U
		...
		is(5.U){
            io.meta_out.bits.pkg_length  := meta_fifo.io.out.bits.pkg_length - (CONFIG.SWRDMA_HEADER_LEN5/8).U
