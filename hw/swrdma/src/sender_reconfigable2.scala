...
class sender_reconfigable2 extends MultiIOModule {
	...
	val PkgProcInst = withClockAndReset(userClk, ~userRstn.asBool){Module(new PkgProc2())}
	val PkgGenInst1 = withClockAndReset(userClk, ~userRstn.asBool){Module(new PkgGen2())}
	val PkgGenInst2 = withClockAndReset(userClk, ~userRstn.asBool){Module(new PkgGen2())}
	...
	withClockAndReset(userClk, ~userRstn.asBool) {
		...
		timer		:= timer + 1.U
		PkgProcInst.io.timer		:= timer
		PkgProcInst.io.base_rtt		:= RegNext(control_reg(219))
		PkgProcInst.io.max_rate		:= RegNext(control_reg(220))
		PkgGenInst1.io.timer		:= timer
		PkgGenInst2.io.timer		:= timer
		PkgGenInst1.io.pkg_num		:= RegNext(control_reg(217))
		PkgGenInst2.io.pkg_num		:= RegNext(control_reg(218))
