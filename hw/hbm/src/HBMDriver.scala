...
class HBM_DRIVER(WITH_RAMA: Boolean=true, IP_CORE_NAME: String="HBMBlackBox", RAMA_CORE_NAME: String="RAMABlackBox") extends Module{
	...
	val io = IO(new Bundle{
		...
		val axi_hbm 		= Vec(32,Flipped(new AXI(33, 256, 6, 0, 4)))
	})

	// AXI bus connected as HBM ports
	val axi_hbm_port = Wire(Vec(32, Flipped(new AXI(33, 256, 6, 0, 4))))

	// Optional RAMA
	var i = 0
	for (i <- 0 until 32) {
		val rama = if (WITH_RAMA) {Some(withClockAndReset(io.hbm_clk, !io.hbm_rstn){Module(new RAMA(IP_CORE_NAME=RAMA_CORE_NAME))})} else None
		if (WITH_RAMA) {
	...
	val instHbm = Module(new HBMBlackBox(IP_CORE_NAME=IP_CORE_NAME))
